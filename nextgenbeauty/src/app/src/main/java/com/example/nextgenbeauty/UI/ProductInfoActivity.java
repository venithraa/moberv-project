...
public class ProductInfoActivity extends AppCompatActivity  implements NavigationView.OnNavigationItemSelectedListener{
    ...
    private void setProductData(){
        ...
        ValueEventListener valueEventListener = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if(snapshot.exists()){
                    for(DataSnapshot dataSnapshot : snapshot.child("bodywash").getChildren()){
                        if(dataSnapshot.getKey().equals(ProductName)){
                            PCategory.setText("Category: bodywash");
                            PAmount.setText("Available Amounts: "+dataSnapshot.child("quantity").getValue());
                            break;}
                    }
                    for(DataSnapshot dataSnapshot : snapshot.child("bathsalt").getChildren()){
                        if(dataSnapshot.getKey().equals(ProductName)){
                            PCategory.setText("Category: bathsalt");
                            PAmount.setText("Available Amounts: "+dataSnapshot.child("quantity").getValue());
                            break;}
                    }

                    for(DataSnapshot dataSnapshot : snapshot.child("soap").getChildren()){
                        if(dataSnapshot.getKey().equals(ProductName)){
                            PCategory.setText("Category: soap");
                            PAmount.setText("Available Amounts: "+dataSnapshot.child("quantity").getValue());
                            break;}
                    }

                    for(DataSnapshot dataSnapshot : snapshot.child("scrub").getChildren()){
                        if(dataSnapshot.getKey().equals(ProductName)){
                            PCategory.setText("Category: scrub");
                            PAmount.setText("Available Amounts: "+dataSnapshot.child("quantity").getValue());
                            break;}
                    }
